...
__global__ void kernel ( FLOAT *result_dev, FLOAT *temp_dev, FLOAT *power_dev, FLOAT *DEBUG) {
...
void compute_tran_temp(FLOAT *result, int num_iterations, FLOAT *temp, FLOAT *power, int row, int col) 
{
    ...
    for (int i = 0; i < num_iterations ; i++)
    {
        ...
        kernel<<<gridDist, blockDist>>> (result_dev, temp_dev, power_dev, DEBUG);
        ...
        if (i == num_iterations-1) err = cudaMemcpyAsync(result, result_dev, (size_t)(sizeof(FLOAT)*col*row), cudaMemcpyDeviceToHost);
